...
# A small in-memory dataset (same as buggy_view)
ALL_PROJECTS = [
...
def buggy_search(request):
    """
    Fixed version of the intentionally buggy view.
    """

    # BUG 1 FIX: safely handle missing 'q'
    query = (request.GET.get('q') or "").strip()

    field_filter = request.GET.get('field', 'All')
    projects = ALL_PROJECTS.copy()

    # BUG 2 FIX: use proper equality instead of identity
    if field_filter != 'All':
        projects = [p for p in projects if p['field'] == field_filter]

    # BUG 3 FIX: correct key name 'description'
    def predicate(p):
        return (
            query.lower() in p['title'].lower()
