...
One-shot bootstrap for the "Discovery Hub" Django project.

What it sets up:
- Virtualenv with: Django (5.x), psycopg2-binary, python-dotenv
- Django project package: discovery_hub
- Two apps: accounts (custom user) and pages
- Custom user model with user_type = university | company | investor + email login
- Minimal templates, partials, and CSS
...
Usage:
    python bootstrap_discovery_hub.py
"""
import os
import sys
import platform
...
def main():
    ...
    run([str(py), "-m", "pip", "install", "Django>=5.0,<6.0", "psycopg2-binary", "python-dotenv", "debugpy"])
        ...
        {% block content %}
          <section class="auth-card">
            ...
            <div class="button-row">
              <a class="btn" href="{% url 'login' %}?type=university">I am a University</a>
              <a class="btn" href="{% url 'login' %}?type=company">I am a Company</a>
              <a class="btn" href="{% url 'login' %}?type=investor">I am an Investor</a>
            </div>
    ...
    # Screen templates with role-partials
    screen1_html = dedent("""\
        {% extends "base.html" %}
        {% block content %}
    ...
    screen2_html = dedent("""\
        ...
        {% block content %}
          <section class="dash-card">
            ...
            {% if request.user.user_type == 'university' %}
              {% include "pages/partials/s2_university.html" %}
            {% elif request.user.user_type == 'company' %}
              {% include "pages/partials/s2_company.html" %}
            {% elif request.user.user_type == 'investor' %}
              {% include "pages/partials/s2_investor.html" %}
            {% else %}
              <p>No role specific content yet.</p>
            {% endif %}
    ...
    screen3_html = dedent("""\
        ...
        {% block content %}
          <section class="dash-card">
            ...
            {% if request.user.user_type == 'university' %}
              {% include "pages/partials/s3_university.html" %}
            {% elif request.user.user_type == 'company' %}
              {% include "pages/partials/s3_company.html" %}
            {% elif request.user.user_type == 'investor' %}
              {% include "pages/partials/s3_investor.html" %}
            {% else %}
              <p>No role specific content yet.</p>
            {% endif %}
    ...
    # Partials for each screen and role
    for s in (1,2,3):
        for role, label in (("university","University"),("company","Company"),("investor","Investor")):
            write(cwd / "templates" / "pages" / "partials" / f"s{s}_{role}.html",
